...
my $soloInputSAMattrBarcodeSeq = "CR UR";
my $soloInputSAMattrBarcodeQual = "-";
my $soloCBstart = 1;
my $soloCBlen = 16;
my $soloUMIstart = 17;
my $soloUMIlen = 10;
...
# Change by JY to fit the .bam file input
#die_usage() unless @ARGV == 3;

if(@ARGV == 2){
   die_usage() unless index($ARGV[1], ".bam")>0;
}else{
   die_usage() unless @ARGV == 3;
}
